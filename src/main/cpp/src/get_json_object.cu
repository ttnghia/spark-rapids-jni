...
__launch_bounds__(block_size, min_block_per_sm) CUDF_KERNEL
  void get_json_object_kernel(cudf::column_device_view input,
                              ...
                              int8_t* max_path_depth_exceeded,
                              bool output_as_string_verbatim)
{
  ...
  if (str.size_bytes() > 0) {
    json_parser p{char_range{str}, output_as_string_verbatim};
                   ...
                   rmm::cuda_stream_view stream)
  {
    ...
    get_json_object_kernel<block_size, min_block_per_sm>
      <<<num_blocks, block_size, 0, stream.value()>>>(
        input, path_data, num_threads_per_row, max_path_depth_exceeded, output_as_string_verbatim);
  }
};
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  kernel_launcher::exec(
    input, d_path_data, d_max_path_depth_exceeded, output_as_string_verbatim, stream);
  ...
  rmm::device_async_resource_ref mr)
{
  ...
  while (starting_path < num_outputs) {
    ...
    auto tmp = get_json_object_batch(
      *d_input_ptr, in_offsets, batch, scratch_size, output_as_string_verbatim, stream, mr);
  ...
  rmm::device_async_resource_ref mr)
{
  CUDF_FUNC_RANGE();
  return std::move(
    detail::get_json_object(input, {instructions}, -1, -1, output_as_string_verbatim, stream, mr)
      .front());
}
